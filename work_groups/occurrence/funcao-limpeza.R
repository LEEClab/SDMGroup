...
setwd("D:\\GitHub\\SDMGroupLocal")
source("funcoes.R")
project = 'pabloplants'
file.csv <- 'pabloplants'

BR.pol <- readShapePoly(fn=paste0(getwd(),"/GISBase/BR.shp"))
#NT.pol <- readShapePoly(fn=paste0(getwd(),"/GISBase/neotropical.shp"))
info_geografica(paste0(getwd(),'/GISBase'))

data.in <- as.data.frame(read_occurrence_records(file.csv,project =project,data_source = 'all', sep=';'),stringsAsFactors = FALSE)
...
limpeza<-function(occ.tmp=data.frame(na),
  ...
  if(check.centroid==T){
    ...
    if (length(iqual)!=0){
      ...
      cat("IDs", set5$ID[iqual], "has the coordinate of the centroid of the municipality")
    }else{
  ...
  set.fin=merge(set4,set5[,c("ID" ,"ok_state","suggest_state","ok_mun","suggest_mun","coord_mun","Longmuncen","Latmuncent", "coord_mun_cent")] ,by="ID",all.x=T)
...
#     
  if(NROW(occ.tmp.out)>0){
      occ.tmp.out=cbind(ID=NA,occ.tmp.out[,-ncol(occ.tmp.out)],ok_country=NA,suggest_country=NA,ok_state=NA, suggest_state=NA,ok_mun=NA,suggest_mun=NA,coord_mun=NA,Longmuncen=NA,Latmuncent=NA,coord_mun_cent=NA)
      occ.tmp.out=rbind(occ.tmp.out, set.fin)
      save_occurrence_records(occ.tmp.out,'limpeza-out',project,'clean_results',sep='\t')} else{
        save_occurrence_records(set.fin,'limpeza-out',project,'clean_results',sep='\t')
      }
