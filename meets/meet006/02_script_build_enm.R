...
setwd("D:/github/SDMGroup/meets/meet006/data")

# importando os pontos de ocorrencia 
points <- read.table("natalia_birdies.txt", h = T, sep = "\t")
str(points)

am <- shapefile("D:/github/SDMGroup/meets/meet006/data/south_america_diss_gcs_wgs84.shp")
am

plot(am, col = "gray80", axes = T, xlim = c(-60, -60), main = "Mapa Natalia Birdies")
points(points[points$sp == "S_lineola", 2], points[points$sp == "S_lineola", 3], pch = 20, add = T)
...
plot(env.stack.0k[[1]])
points(points[, 2], points[, 3], pch = 20, col = points$sp)
